...
class AgentManageServices extends BaseServices
{
     ...
     * @return array
     */
    public function agentSystemPage(array $where)
    {
        /** @var UserServices $userServices */
        $userServices = app()->make(UserServices::class);
        $data = $userServices->getAgentUserList($where);
        $broken_time = intval(sys_config('extract_time'));
        $search_time = time() - 86400 * $broken_time;
        //改造时间
        $search_time = '1970/01/01' . ' - ' . date('Y/m/d H:i:s', $search_time);
        $spreadUids = array_unique(array_column($data['list'], 'spread_uid'));
        $uids = array_column($data['list'], 'uid');
        /** @var UserBillServices $userBill */
        $userBill = app()->make(UserBillServices::class);
        $userBrokerageGetlist = $userBill->getUsersBokerageSum(['uid' => $uids, 'pm' => 1], $search_time);
        $userBrokerageUselist = $userBill->getUsersBokerageSum(['uid' => $uids, 'pm' => 0], $search_time);
        ...
        foreach ($data['list'] as &$item) {
            ...
            //返佣 +
            $brokerage_commission = $userBrokerageGetlist[$item['uid']] ?? '0';
            //退款退的佣金 -
            $refund_commission = $userBrokerageUselist[$item['uid']] ?? '0';
    ...
    public function getSpreadBadge($where)
    {
        ...
        if ($data['sum_count']) {
            ...
            $data['pay_price'] = 0;
            //提现次数
            $data['extract_count'] = app()->make(UserExtractServices::class)->getCount([['uid', 'in', $uids]]);
        }
        return [
            [
                'name' => '分销员人数(人)',
    ...
    public function getStairOrderList(int $uid, array $where)
    {
        ...
        if ($orderIds) {
            /** @var UserBillServices $userBill */
            $userBill = app()->make(UserBillServices::class);
            $userBillList = $userBill->getColumn([['category', '=', 'now_money'], ['type', '=', 'brokerage'], ['link_id', 'IN', $orderIds]], 'number', 'link_id');
     ...
     */
    public static function wechatCode(int $uid)
    {
        /** @var QrcodeServices $qrcode */
        $qrcode = app()->make(QrcodeServices::class);
        $code = $qrcode->getOne(['third_id' => $uid, 'third_type' => 'spread']);
        if (empty($code)) {
    ...
    public function lookXcxCode(int $uid)
    {
        ...
        if (!$imageInfo) {
            /** @var QrcodeServices $qrcode */
            $qrcode = app()->make(QrcodeServices::class);
            $resForever = $qrcode->getOne(['third_id' => $uid, 'third_type' => 'spread']);
            if ($resForever) {
