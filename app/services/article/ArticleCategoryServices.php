...
class ArticleCategoryServices extends BaseServices
{
    ...
    public function createForm(int $id)
    {
        ...
        $f[] = Form::input('title', '分类名称', $info['title'] ?? '');
        $f[] = Form::input('intr', '分类简介', $info['intr'] ?? '')->type('textarea');
        $f[] = Form::frameImageOne('image', '分类图片', Url::buildUrl('admin/widget.images/index', array('fodder' => 'image')), $info['image'] ?? '')->icon('ios-add')->width('60%')->height('435px');
        $f[] = Form::number('sort', '排序', $info['sort'] ?? 0);
