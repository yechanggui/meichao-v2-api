...
class SystemGroupDataServices extends BaseServices
{
    ...
    public function getGroupDataList(array $where)
    {
        ...
        $header = $services->getValueFields((int)$where['gid']);
        $title = [];
    ...
    public function createGroupForm(int $gid, array $groupData = [])
    {
        ...
        foreach ($fields as $key => $value) {
            ...
            switch ($value["type"]) {
                ...
                case 'upload':
                    ...
                    $f[] = Form::frameImageOne($value["title"], $value["name"], $this->url('admin/widget.images/index', ['fodder' => $value["title"], 'big' => 1], true), $image)->icon('ios-image')->width('60%')->height('435px');
                    break;
                case 'uploads':
                    if ($fvalue) {
                    ...
                    $f[] = Form::frameImages($value["title"], $value["name"], $this->url('admin/widget.images/index', ['fodder' => $value["title"], 'big' => 1], true), $images)->maxLength(5)->icon('ios-images')->width('60%')->height('435px')->spin(0);
                    break;
                default:
                    $f[] = Form::input($value["title"], $value["name"], $fvalue);
                    break;
        ...
        $f[] = Form::number('sort', '排序', $groupData["sort"] ?? 1);
        $f[] = Form::radio('status', '状态', (string)($groupData["status"] ?? 1))->options([['value' => 1, 'label' => '显示'], ['value' => 0, 'label' => '隐藏']]);
        return $f;
    }
