...
class StoreProductReplyServices extends BaseServices
{
    ...
    public function getRecProductReply(int $productId)
    {
        ...
        if ($res) {
            $res['cartInfo'] = json_decode($res['cartInfo'], true) ?: [];
    ...
    {
        $replyCount = $this->dao->count(['product_id' => $id]);
        if ($replyCount) {
            $goodReply = $this->dao->count(['product_id' => $id, 'product_score' => 5]);
            if ($goodReply) {
    ...
    {
        $list = $this->dao->replyList($id, $type);
