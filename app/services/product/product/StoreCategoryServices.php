...
class StoreCategoryServices extends BaseServices
{
    ...
    {
        return [
            Form::select('pid', '父级', (string)($info['pid'] ?? ''))->setOptions($this->menus())->filterable(1)->required(),
            Form::input('cate_name', '分类名称', $info['cate_name'] ?? '')->maxlength(30)->required(),
            Form::frameImageOne('pic', '分类图标(180*180)', Url::buildUrl('admin/widget.images/index', array('fodder' => 'pic')), $info['pic'] ?? '')->icon('ios-add')->width('60%')->height('435px'),
            Form::number('sort', '排序', $info['sort'] ?? 0)->min(0),
            Form::radio('is_show', '状态', $info['is_show'] ?? 1)->options([['label' => '显示', 'value' => 1], ['label' => '隐藏', 'value' => 0]])
        ];
    }
