...
class LoginServices extends BaseServices
{
    ...
    public function login($account, $password, $spread)
    {
        ...
        if ($user) {
            ...
            if ($user->pwd === md5('123456'))
                throw new ValidateException('请修改您的初始密码，再尝试登陆！');
        } else {
            throw new ValidateException('账号或密码错误');
        }
        if (!$user['status'])
    ...
    public function updateUserInfo($user, $userInfo)
    {
        ...
        $userInfo->avatar = $user['headimgurl'] ?? $userInfo->avatar;
        $userInfo->phone = $user['phone'] ?? $userInfo->phone;
        $userInfo->last_time = time();
        $userInfo->last_ip = app()->request->ip();
        if ($userInfo->spread_uid) {
            $userInfo->login_type = $user['login_type'] ?? $userInfo->login_type;
        } else {
            //绑定分销关系 = 所有用户
            if (sys_config('brokerage_bindind', 1) == 1) {
                ...
                if ($spreadUid) {
                    ...
                    $userInfo->spread_time = $userInfo->last_time;
                    $this->dao->incField((int)$spreadUid, 'spread_count', 1);
                }
            }
        }
        if (!$this->dao->update($userInfo['uid'], $userInfo->toArray(), 'uid')) {
    ...
    public function verify(SmsSendServices $services, $phone, $type, $time, $ip)
    {
        ...
        if (CacheService::get('code_' . $phone))
            throw new ValidateException($time . '秒内有效');
        $code = rand(100000, 999999);
        $data['code'] = $code;
        $res = $services->send(true, $phone, $data, 'VERIFICATION_CODE');
        if ($res !== true)
     ...
     */
    public function register($account, $password, $spread)
    {
    ...
    {
        $user = $this->dao->getOne(['account' => $account]);
        if (!$user) {
            throw new ValidateException('用户已存在');
        }
        return $this->dao->resetPwd($user['uid'], md5((string)$password));
    }
     ...
     * @throws \think\db\exception\ModelNotFoundException
     */
    public function mobile($phone, $spread)
    {
        //数据库查询
        $user = $this->dao->getOne(['phone' => $phone]);
        if (!$user) {
            $user = $this->register($phone, '123456', 0);
            if (!$user) {
                throw new ValidateException('用户登陆失败,无法生成新用户,请稍后再试!');
            }
        }
