...
class UserLevelServices extends BaseServices
{
    ...
    {
        $userLevelInfo = $this->dao->getUserLevel($uid, 'id,level_id,add_time');
        $data = [];
        if ($userLevelInfo) {
            ...
            $data['incon'] = $userLevelInfo['levelInfo']['incon'] ?? '';
    ...
    public function edit(int $id)
    {
        ...
        $field[] = Form::number('discount', '享受折扣', isset($vipinfo) ? $vipinfo->discount : 100)->min(0)->col(12)->placeholder('输入折扣数100，代表原价，90代表9折');
        $field[] = Form::number('exp_num', '等级经验值', isset($vipinfo) ? $vipinfo->exp_num : 0)->min(0)->col(12);
