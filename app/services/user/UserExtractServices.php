...
class UserExtractServices extends BaseServices
{
    ...
    public function getUserExtractList(array $where, string $field = '*')
    {
        ...
        if (isset($where['uid']) && $where['uid'] != '') {
            $whereData['uid'] = $where['uid'];
        }
        if (isset($where['status']) && $where['status'] != '') {
            $whereData['status'] = $where['status'];
        }
        if (isset($where['extract_type']) && $where['extract_type'] != '') {
            $whereData['status'] = $where['status'];
        }
        if (isset($where['nireid']) && $where['nireid'] != '') {
            $whereData['like'] = $where['nireid'];
        }
        if (isset($where['data']) && $where['data'] != '') {
            $whereData['time'] = $where['data'];
        }
        [$page, $limit] = $this->getPageValue();
        $list = $this->dao->getExtractList($whereData, $field, $page, $limit);
    ...
    public function getExtractSum(array $where)
    {
        ...
        return $this->dao->getWhereSumField($whereData, 'extract_price');
    }
    ...
    public function changeFail(int $id, $userExtract, $message)
    {
        ...
        $userServices->addNowMoney($uid, $user['now_money'], $extract_number);
    ...
    public function changeSuccess(int $id, $userExtract)
    {
        ...
        if (strtolower($userExtract['recharge_type']) == 'routine') {
    ...
    public function index(array $where)
    {
        ...
        //未提现金额
        $extract_statistics['brokerage_not'] = $userServices->getSumBrokerage(['time' => $where['data']]);
    ...
    public function refuse(int $id, string $message)
    {
        ...
        if (!$extract) {
            return $this->fail('操作记录不存在!');
        }
        if ($extract->status == 1) {
            return $this->fail('已经提现,错误操作');
        }
        if ($extract->status == -1) {
            return $this->fail('您的提现申请已被拒绝,请勿重复操作!');
        }
    ...
    public function adopt(int $id)
    {
        $extract = $this->getExtract($id);
        if (!$extract) {
            return $this->fail('操作记录不存!');
        }
        if ($extract->status == 1) {
            return $this->fail('您已提现,请勿重复提现!');
        }
        if ($extract->status == -1) {
            return $this->fail('您的提现申请已被拒绝!');
        }
    ...
    public function cash(int $uid, array $data)
    {
        ...
        if ($data['money'] > $commissionCount) return app('json')->fail('可提现佣金不足');
        ...
        $bill_data = ['title' => '余额提现', 'link_id' => $res1['id'], 'balance' => $user['brokerage_price'], 'number' => $data['money'], 'mark' => $mark];
