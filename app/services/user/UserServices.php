...
class UserServices extends BaseServices
{
     ...
     * @throws \think\db\exception\ModelNotFoundException
     */
    public function getUserLablel($uid)
    {
        $userlabels = $this->dao->getUserLabel($uid, 'uid');
        if (is_array($uid))
            $userlabels = array_combine(array_column($userlabels, 'uid'), $userlabels);
        return $userlabels;
    }
    ...
    public function index(array $where)
    {
        ...
        if ($list) {
            ...
            foreach ($list as &$item) {
                ...
                //用户类型
                if ($item['openid'] != '' && $item['user_type'] != '') {
                    $item['user_type'] = '通用';
                } else if ($item['openid'] != '' && $item['w_user_type'] == 'routine') {
                    $item['user_type'] = '小程序';
                ...
                $item['labels'] = isset($userlabel[$item['uid']]['label']) ? implode(',', array_column($userlabel[$item['uid']]['label'], 'label_name')) : '';
            }
        }
    ...
    public function saveSetLabel($uids, $lable_id)
    {
        ...
        if (!app()->make(UserLabelRelationServices::class)->setUserLable($uids, $lable_id)) {
    ...
    public function cleanUpLevel($uid)
    {
        if (!$this->getUserInfo($uid))
            throw new AdminException('用户不存在');
        app()->make(UserLevelServices::class)->delUserLevel($uid);
        if (!$this->dao->update($uid, ['clean_time' => time(), 'level' => 0], 'uid'))
    ...
    public function getHeaderList(int $uid, $userInfo = [])
    {
        ...
        return [
            ...
            [
                'title' => '总计订单',
                'value' => $orderData['orderCountNum'] ?? 0,
                'key' => '笔',
            ],
            [
                'title' => '总消费金额',
                'value' => $orderData['orderTotalPrice'] ?? 0,
                'key' => '元',
            ],
            [
                'title' => '积分',
            ...
            [
                'title' => '本月订单',
                'value' => $orderData['orderCountNumMonth'] ?? 0,
                'key' => '笔',
            ],
            [
                'title' => '本月消费金额',
                'value' => $orderData['orderTotalPricemonth'] ?? 0,
                'key' => '元',
            ]
        ];
    }
     ...
     * @return mixed
     */
    public function oneUserInfo(int $id, $type)
    {
        switch ($type) {
            case 'spread':
                ...
                break;
            case 'order':
                return app()->make(StoreOrderServices::class)->getUserOrderList($id);
                break;
            case 'integral':
                return app()->make(UserBillServices::class)->getIntegralList($id, [], 'title,number,balance,mark,FROM_UNIXTIME(add_time,"%Y-%m-%d") as add_time');
                break;
            case 'sign':
                return app()->make(UserBillServices::class)->getSignList($id, [], 'title,number,mark,FROM_UNIXTIME(add_time,"%Y-%m-%d") as add_time');
                break;
            case 'coupon':
                return app()->make(StoreCouponUserServices::class)->getUserCouponList($id);
                break;
            case 'balance_change':
                return app()->make(UserBillServices::class)->getBrokerageList($id, [], 'title,type,number,balance,mark,pm,status,FROM_UNIXTIME(add_time,"%Y-%m-%d") as add_time');
                break;
            default:
