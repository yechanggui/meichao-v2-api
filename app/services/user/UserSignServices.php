...
class UserSignServices extends BaseServices
{
    ...
    public function sign(int $uid)
    {
        ...
        //增加签到数据
        $this->setSignData($uid, $title, $sign_num, $user['integral'], $user['exp'], $exp_num);

        $user->integral = $user->integral + $sign_num;
        $user->exp = $user->exp + $exp_num;
        if (!$user->save()) {
    ...
    public function signUser(int $uid, $sign, $integral, $all)
    {
        ...
        if ($integral || $all) {
            /** @var UserBillServices $userBill */
            $userBill = app()->make(UserBillServices::class);
            $user['sum_integral'] = $userBill->getRecordCount($user['uid'], 'integral', 'sign,system_add,gain');
            $user['deduction_integral'] = $userBill->getRecordCount($user['uid'], 'integral', 'deduction', '', true) ?? 0;
            $user['today_integral'] = $userBill->getRecordCount($user['uid'], 'integral', 'sign,system_add,gain', 'today');
        }
        unset($user['pwd']);
        if (!$user['is_promoter']) {
    ...
    public function getSignMonthList($uid)
    {
        ...
        foreach ($data as $item) {
            ...
            $value['list'] = array_filter($dataIdsList, function ($val) use ($item) {
