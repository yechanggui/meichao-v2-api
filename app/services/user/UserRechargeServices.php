...
class UserRechargeServices extends BaseServices
{
     ...
     * @return array
     */
    public function getRechargeList(array $where, string $field = '*')
    {
        $whereData = [];
        if (isset($where['data'])) {
            $whereData['time'] = $where['data'];
...
//        $refund_data['refund_account']='REFUND_SOURCE_RECHARGE_FUNDS';
        ...
        $userServices->cutNowMoney($UserRecharge['uid'], $userInfo['now_money'], $refund_price);
        ...
        switch (strtolower($UserRecharge['recharge_type'])) {
            ...
            case 'routine':
                ...
                Queue::instance()->do('sendRechargeSuccess')->job(RoutineTemplateJob::class)->data($openid, $UserRecharge, $refund_price)->push();
                break;
        }
        $billData = ['title' => '系统退款', 'number' => $refund_price, 'link_id' => $id, 'balance' => $userInfo['now_money'], 'mark' => '退款给用户' . $refund_price . '元'];
    ...
    public function importNowMoney(int $uid, $price)
    {
        ...
        $bill_data = ['title' => '用户佣金转入余额', 'link_id' => 0, 'number' => $price, 'balance' => $user['now_money'], 'mark' => '成功转入余额' . floatval($price) . '元'];
        /** @var UserBillServices $userBill */
        $userBill = app()->make(UserBillServices::class);
        $userBill->expendNowMoney($uid, 'recharge', $bill_data);
        $extractInfo = [
            'uid' => $uid,
        ...
        if (!$userExtract->save($extractInfo)) {
            throw new ValidateException('写入余额提现失败');
        }
        return true;
    }
    ...
    public function rechargeSuccess($orderId)
    {
        ...
        if (!$userServices->update((int)$order['uid'], ['now_money' => bcadd((string)$user['now_money'], (string)$price, 2)], 'uid')) {
            throw new ValidateException('修改用户信息失败');
        }
        return true;
    }
