...
class StoreCouponUserDao extends BaseDao
{
    ...
    public function productIdsByCoupon(array $productIds, int $uid, string $price)
    {
        ...
        })->where(['uid' => $uid, 'is_fail' => 0, 'status' => 0])->order('coupon_price DESC')
    ...
    public function cateIdsByCoupon(array $cateIds, int $uid, string $price)
    {
        return $this->getModel()->whereIn('cid', function ($query) use ($cateIds) {
            ...
            ->order('coupon_price DESC')->select()->hidden([
                'status', 'is_fail'
            ])->toArray();
    }
    ...
    public function getUserCoupon(array $ids, int $uid, string $price)
    {
        ...
        })->where('use_min_price', '<=', $price)->order('coupon_price DESC')->select()->hidden([
            'status', 'is_fail'
        ])->toArray();;
    }
    ...
    {
        return $this->search($where)->page($page, $limit)->when($order != '', function ($query) use ($order) {
            $query->order($order);
        })->select()->toArray();
    }
}
