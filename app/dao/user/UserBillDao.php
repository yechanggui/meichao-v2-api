...
class UserBilldao extends BaseDao
{
    ...
    {
        return $this->search($where)->field($field)->page($page, $limit)->order('id desc')->select()->toArray();
    }
    ...
    {
        return $this->getModel()->where($where)->sum('number');
    }
    ...
    {
        return $this->search($where)->group('uid')->count();
    }
