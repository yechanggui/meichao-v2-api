...
class StoreProductDao extends BaseDao
{
    ...
    {
        return $this->search($where)->with(['couponId', 'description'])->when($page != 0, function ($query) use ($page, $limit) {
            $query->page($page, $limit);
    ...
    public function getRecommendProduct(string $field, int $num = 0, int $page = 0, int $limit = 0)
    {
        ...
        return $this->search($where)->with('couponId')
            ->field(['id', 'image', 'store_name', 'cate_id', 'price', 'ot_price', 'IFNULL(sales,0) + IFNULL(ficti,0) as sales', 'unit_name', 'sort', 'activity'])
